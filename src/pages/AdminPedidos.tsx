...
import { Check, ChevronDown, ChevronUp, Clock3, PackageCheck, Search, ShoppingBag, Warehouse, XCircle } from "lucide-react";
...
type AdminOrder = { id:string; invoice_number?:string; created_at:string; subtotal?:number; tax_rate?:number; tax_amount?:number; total:number; status:"pending"|"paid"|"cancelled"; name:string; email:string; phone?:string; customer_id:string; purchase_count:number; delivery:Record<string,string>; items:OrderItem[] };
...
const AdminPedidos = () => {
  ...
  return <PageLayout><main className="container pt-32 pb-24 min-h-[70vh]">
    <header className="mb-9 flex flex-col md:flex-row md:items-end justify-between gap-5"><div><p className="text-xs tracking-[.35em] uppercase text-primary">Administración</p><h1 className="font-serif text-5xl mt-2">Solicitudes de clientes</h1><p className="text-muted-foreground mt-3">Busca pedidos, revisa sus detalles y confirma las compras pagadas.</p></div><nav className="flex flex-wrap gap-3"><Link to="/admin/inventario" className="inline-flex items-center gap-2 border border-border px-5 py-3 text-xs uppercase tracking-wider"><Warehouse size={16}/> Inventario</Link><Link to="/admin/opciones-ramo" className="inline-flex items-center gap-2 border border-border px-5 py-3 text-xs uppercase tracking-wider">Opciones del ramo</Link></nav></header>
