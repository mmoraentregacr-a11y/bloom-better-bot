...
export async function notifyAdmins(order:OrderNotification):Promise<boolean> {
  ...
  if(!connectionString||!senderAddress||recipients.length===0){console.warn("Pedido guardado sin correo: faltan variables de Azure Communication Services Email.");return false;}
  ...
  const html=`<div style="font-family:Arial,sans-serif;max-width:720px;margin:auto;color:#173828"><h1>Nueva solicitud de pedido</h1><p><strong>Orden:</strong> ${escapeHtml(order.invoiceNumber)}<br><strong>Cliente:</strong> ${escapeHtml(order.customer.name)} (${escapeHtml(order.customer.email)})<br><strong>Fecha:</strong> ${escapeHtml(new Date(order.createdAt).toLocaleString("es-CR"))}</p><table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:10px">Producto</th><th>Cantidad</th><th style="text-align:right">Precio</th><th style="text-align:right">Importe</th></tr></thead><tbody>${itemRows}</tbody></table><div style="margin-top:20px;text-align:right"><p>Subtotal: <strong>${crc(order.subtotal)}</strong></p><p>IVA (${order.taxRate*100}%): <strong>${crc(order.taxAmount)}</strong></p><p style="font-size:20px">Total: <strong>${crc(order.total)}</strong></p></div><h2>Entrega</h2><ul>${deliveryRows}</ul><p>La solicitud ya está disponible en el panel administrativo de Golden Bloom.</p></div>`;
  const client=new EmailClient(connectionString);
  const poller=await client.beginSend({senderAddress,content:{subject:`Nueva solicitud ${order.invoiceNumber} — Golden Bloom`,plainText:`Nueva solicitud ${order.invoiceNumber}. Cliente: ${order.customer.name} (${order.customer.email}). Total: ${crc(order.total)}.`,html},recipients:{to:recipients.map(address=>({address}))}});
